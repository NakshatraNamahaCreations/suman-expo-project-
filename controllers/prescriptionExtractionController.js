...
/**
 * Main fixed OCR parser.
 * This avoids current medicine picking next medicine's duration/frequency/quantity.
 */
function extractMedicineRowsFromPrescription(text) {
  const rxLines = getPrescriptionSectionLines(text);

  const medicineLines = rxLines.filter((line) => looksLikeMedicineLine(line));
  ...
  for (const line of rxLines) {
    const isMedicine = looksLikeMedicineLine(line);
    const hasInlineFrequency = !!cleanFrequency(line);
    const hasInlineDuration = !!cleanDurationLabel(line);

    /**
     * Case 1:
     * OCR gives full row in one line:
     * TABLET ABC 1 Tablet 1-0-1 After Food 6 Month(s)
     *
     * Case 2:
     * OCR gives column-wise lines:
     * Medicine names separate, dose separate, frequency separate.
     */
    if (!isMedicine || hasInlineFrequency || hasInlineDuration) {
      const dose = cleanDose(line);
      if (dose) doseValues.push(dose);

      const frequency = cleanFrequency(line);
      if (frequency) frequencyValues.push(frequency);

      const instruction = cleanInstruction(line);
      if (instruction) instructionValues.push(instruction);

      const duration = cleanDurationLabel(line);
      if (duration) durationValues.push(duration);
    }
  }

  const medicines = [];

  for (let index = 0; index < medicineLines.length; index++) {
    const currentLine = medicineLines[index];

    const medicineName = extractMedicineNameFromBlock(currentLine);

    if (!medicineName || medicineName.length < 3) continue;

    const inlineDose = cleanDose(currentLine);
    const inlineFrequency = cleanFrequency(currentLine);
    const inlineInstruction = cleanInstruction(currentLine);
    const inlineDurationLabel = cleanDurationLabel(currentLine);

    const dose = inlineDose || doseValues[index] || "1 Tablet";
    const frequency = inlineFrequency || frequencyValues[index] || "";
    const instruction = inlineInstruction || instructionValues[index] || "";
    const durationLabel = inlineDurationLabel || durationValues[index] || "";
    ...
    medicines.push({
      ...
      // duration as days for app calculation
      duration: durationDays,
      durationDays,

      // original prescription label for display
      durationLabel,

      // prescription order quantity
      prescriptionQty: calculatedQty || null,
      calculatedQty: calculatedQty || 0,
      quantity: calculatedQty || 0,
      orderQty: calculatedQty || 0,
      requiredQty: calculatedQty || 0,
    });
  }
...
function getMedicineMatchScore(ocrName = "", dbName = "") {
   ...
   * This avoids wrong match like HEART OPEN matching unrelated medicine.
   */
  if (ocrTokens[0] !== dbTokens[0]) {
    return 0;
  }
    ...
    for (const ocrMed of extractedMedicines) {
      ...
      if (bestMatch && bestScore >= MIN_MATCH_SCORE) {
        ...
        matched.push({
          ...
          /**
           * IMPORTANT:
           * qty = DB stock quantity.
           * Do not use qty for prescription order quantity.
           */
          qty: bestMatch.qty || 0,
          stock: bestMatch.qty || 0,
          inStock: (bestMatch.qty || 0) > 0,
          ...
          /**
           * Prescription order quantity.
           * Frontend la order quantity ku indha fields use pannu.
           */
          prescriptionQty: calculatedQty || null,
          calculatedQty: calculatedQty || 0,
          quantity: calculatedQty || 0,
